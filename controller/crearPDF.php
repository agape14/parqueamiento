...
Operación  sujeta al sistma de pago de obligaciones tributarios con el gobierno central SPOT, sujeta a detracción del 10% si es mayor a S/.700.00
        ...
        $dompdf->setPaper(array(0, 0, 164.409, 0), 'portrait');
...
function pdfFacBolTicket($ruta, $cab, $detalle, $hash)
{
    ...
    try {
        ...
        $hora             = time();
        $fec_ingreso      = date("d/m/Y", strtotime($cab['FechaIngreso']));
        $fec_salida       = date("d/m/Y", strtotime($cab['FechaSalida']));
        $fec_hora_ingreso = $fec_ingreso . ' ' . $cab['HoraIngreso'];
        $fec_hora_salida  = $fec_ingreso . ' ' . $cab['HoraSalida'];
        $text             = $cab['NRO_DOCUMENTO_EMPRESA'] . '|' . $cab['COD_TIPO_DOCUMENTO'] . '|' . str_replace('-', '|', $cab['NRO_COMPROBANTE']) . '|' . $cab['TOTAL_IGV'] . '|' . $cab['TOTAL'] . '|' . $cab['FECHA_DOCUMENTO'] . '|' . $cab['TIPO_DOCUMENTO_CLIENTE'] . '|' . $cab['NRO_DOCUMENTO_CLIENTE'] . '|' . $hash;
        QRcode::png($text, dirname(__FILE__) . '/' . $ruta . '.PNG', 'Q', 15, 0);
...
<body>
    <div class="ticket">
        <p class="centrado">' . $cab['NOMBRE_COMERCIAL_EMPRESA'] . '
            <br>De: ' . $cab['RAZON_SOCIAL_EMPRESA'] . '
            <br>RUC: ' . $cab['NRO_DOCUMENTO_EMPRESA'] . '
            <br>' . $cab['DIRECCION_EMPRESA'] . '
            <br><strong>' . $tipoDocumento . '</strong>
            <br><strong>' . $cab['NRO_COMPROBANTE'] . '</strong>' .
        '<br>-------------------------------------------
        </p>
        <p>
            Fecha: ' . date("d/m/Y", strtotime($cab['FECHA_DOCUMENTO'])) . '
            <br>Moneda: ' . $tipoMoneda . ' Placa: ' . $cab['PlacaTicket'] . '
            <br>Nro. Ticket: ' . $cab['CodigoTicket'] . '
            <br>Entrada: ' . $fec_hora_ingreso . '
            <br>Salida: ' . $fec_hora_salida . '
            <br>-------------------------------------------
        ...
        <p>SUB. TOTAL: ' . $cab['SUB_TOTAL'] . '
            ...
            <br>TOTAL: ' . $cab['TOTAL'] .
            '</p>
        <p><strong>SON: ' . $cab['TOTAL_LETRAS'] . '</strong></p>
        <p >Representación impresa de la ' . $tipoDocumento . '
            <br>' . $hash . '</p>
        <div class="centrado"><img src="' . $ruta . '.PNG" /></div>
    </div>
</body>
        ...
        $dompdf      = new DOMPDF();
        $customPaper = array(0, 0, 210, 841.89);
