...
while ($row_ent = mysqli_fetch_array($entradas)) {
    $fecha_ent    = date('Y-m-d'); //$row_ent['fecha'];
    $tip_doc_cli  = 1;
    ...
    $total        = $row_ent['pago'];
    $subtotal     = number_format((float) round(($total / (1.18)), 2, PHP_ROUND_HALF_DOWN), 2, '.', ',');
    $igv          = $total - $subtotal;
    $totfn        = $totfn + $total;
    $igvfn        = $igvfn + $igv;
            ...
            <div class="panel-body">
                ...
                <div class="card-body ">
                 <div class="row mb-4" >
                     <div class="col-sm-6 " style="background-color: #EAF2F8;border-radius: 5px;margin-top: 1em;">
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col col-sm-8">
                                ...
                                <input type="text" id="txtNRO_DOCUMENTO_EMPRESA" onkeyup="BuscarCliente()" name="txtNRO_DOCUMENTO_EMPRESA" value="<?php echo $nro_doc_emp; ?>" size="8" class="form-control" required/>
                            </div>
                        </div>
                     ...
                     <div class="col-sm-6 " style="background-color:#F4ECF7;border-radius: 5px;margin-top: 1em;">
                         ...
                         <div class="form-group row">
                            ...
                            <div class="col col-sm-8">
                                <label>Nro. Documento:</label>
                                <input type="text" id="txtRUC" onkeyup="BuscarCliente()" name="txtRUC" value="<?php echo $nro_doc_cli; ?>" size="11" onKeyPress="return soloNumeros(event)"  class="form-control" required/>
                                <input type="hidden" id="txtRucBol" name="txtRucBol" class="form-control" value="<?php echo $nro_doc_cli; ?>"/>
                            </div>
                        </div>
                         <label>Razon Social:</label>
                         <input type="text" id="txtRAZON_SOCIAL" name="txtRAZON_SOCIAL" class="form-control" value="<?php echo $nom_cli . ' ' . $ape_cli; ?>" size="30"  placeholder="RAZON SOCIAL" required/>
                         <input type="hidden" id="txtRazonSocialBol" name="txtRazonSocialBol" class="form-control" value="<?php echo $nom_cli . ' ' . $ape_cli; ?>"/>
                         <label>Direccion:</label>
                         <input type="text" id="txtDIRECCION" name="txtDIRECCION" class="form-control" value="<?php echo $dir_cli; ?>"  size="30" />
                         <input type="hidden" id="txtDireccionBol" name="txtDireccionBol" class="form-control" value="<?php echo $dir_cli; ?>"/>
                 ...
                 <div class="table-responsive-sm">
                    <input type="hidden" id="txtFacturacion_id" name="txtFacturacion_id" >
                    <h3>Detalle:</h3>
...
while ($r_entd = mysqli_fetch_array($entr_det)) {?>
                             <tr>
                                 ...
                                 <td >NIU</td>
                                 <td ><?php echo number_format(($r_entd['pago'] / 1.18), 2, '.', ',') ?></td>
                                 <td >1</td>
                                 <td ><?php echo number_format(($r_entd['pago'] / 1.18), 2, '.', ',') ?></td>
                                 <td ><?php echo number_format($r_entd['pago'] - ($r_entd['pago'] / 1.18), 2, '.', ',') ?></td>
                        ...
                        <tfoot>
                            <tr>
                                <td colspan="7" align="right" style="font-weight: bold;">Subtotal</td>
                                <td align="right" style="font-weight: bold;"><?php echo number_format($subtotal, 2, '.', ','); ?></td>
                            </tr>
                            <tr>
                                <td colspan="7" align="right" style="font-weight: bold;">IGV (18%)</td>
...
<div id="loading" style="display: none">
    <img src="../../dist/img/cargando.gif" alt="loader">
</div>
   ...
   <script>
      ...
      function GuardarBoleta(){
        ...
        for (i = 0; i < lista.length; i++) {
            ...
            detalle["txtITEM"] = i + 1;
            detalle["txtUNIDAD_MEDIDA_DET"] = lista[i][2].toString();
            detalle["txtCANTIDAD_DET"] = lista[i][4].toString();
            detalle["txtPRECIO_DET"] = lista[i][7].toString();//PRECIO CON IGV
            detalle["txtIMPORTE_DET"] = (redondeo((lista[i][7]/1.18),2) ).toString();//lista[i][7].toString();//txtPRECIO_SIN_IGV_DET*txtCANTIDAD_DET
            detalle["txtPRECIO_TIPO_CODIGO"] = "01";
            detalle["txtIGV"] = (redondeo((lista[i][7]-redondeo((lista[i][7]/1.18),2)),2)).toString();//lista[i][6].toString();
            detalle["txtISC"] = "0";
            detalle["txtCOD_TIPO_OPERACION"] = "10";
            detalle["txtCODIGO_DET"] = "00"+lista[i][0].toString();
            detalle["txtDESCRIPCION_DET"] = lista[i][1].toString();
            detalle["txtCOD_SUNAT"]="",
            detalle["txtPRECIO_SIN_IGV_DET"] = (redondeo((lista[i][7]/1.18),5)).toString();//lista[i][3].toString();//PRECIO SIN IGV
            detalle["txtSUB_TOTAL_DET"] = lista[i][5].toString();
            //redondeo(parseFloat(lista[i][5]) / parseFloat(1.18),2);
            DATA.push(detalle);
        }
                ...
                //if (response.cod_sunat == "0") {
                    ...
                    $.ajax({
                        ...
                        success: function (response) {
                            ...
                            $("#txtFacturacion_id").val(response);
                        },
                        error: function (data) {
                            $('#loading').fadeOut();
                            console.log('DAtos');
      ...
      function ImprimirBoleta(){
        var facturacion_id=$("#txtFacturacion_id").val();
        location.href='ver_comprobante.php?facturacion_id='+facturacion_id;
      ...
      function ValidarRUC(rucVal) {
        ...
        function MotivoNotaCD() {
            ...
            if (TipoComprobate == '01') {
                ...
                $('#txtRAZON_SOCIAL').attr("disabled", false);
                $('#txtDIRECCION').attr("disabled", false);
                $('#txtRUC').focus();
                //================documento identidad================
                $('#txtTIPO_DOCUMENTO_CLIENTE').val("6");
            } else if (TipoComprobate == '03') {
                $('#txtRUC').attr('maxlength','8');
                $('#txtRUC').val($('#txtRucBol').val());
                $('#txtRAZON_SOCIAL').val($('#txtRazonSocialBol').val());
                $('#txtDIRECCION').val($('#txtDireccionBol').val());
                //$('#txtNRO_DOC_MODIFICA').attr("disabled", false);
                //================documento identidad================
                $('#txtTIPO_DOCUMENTO_CLIENTE').val("1");
            }
            serie_numero();
        ...
        function serie_numero() {
            var TipoComprobate = $('#txtID_TIPO_DOCUMENTO').val();
            $.ajax({
                url: "getSerieNroComprobante.php",
                type: "post",
                dataType: 'json',
                data: {tipo:TipoComprobate},
                success: function (response) {
                    console.log('Rpta Tip Doc');
                    console.log(response);
                    $("#txtSERIE").val(response[0].serieDoc);
                    $("#txtNUMERO").val(response[0].correlativo);
                    /*$("#btnGuardar").prop('disabled', true);
                    $("#btnImprimir").prop('disabled', false);
                    $("#txtFacturacion_id").val(response);*/
                },
