                        ...
                        <div class="panel-body">
                            <div class="tab-content">
                                <div role="tabpanel" class="tab-pane active" id="tab1primary">
                                    <form method="post" action="listado_facturacion.php" enctype="multipart/form-data" class="form-horizontal">
...
?>
    ...
    $query    = mysqli_query($con, "SELECT  f.facturacion_id, f.tipo,CONCAT( f.serie,'-',f.numero) serienro, f.serie,f.numero, date_format(f.fecha, '%d/%m/%Y') fecha,SUBSTR(f.hora_comprobante,1,5) horacomp, f.cliente_nom, f.facturacion_subtotal,f.facturacion_igv, f.facturacion_total, f.facturacion_estado,CASE WHEN f.facturacion_estado='0' THEN 'Aceptado' WHEN f.facturacion_estado='0' THEN f.facturacion_estado_descripcion ELSE '' END fact_estado,CASE WHEN f.facturacion_emitido=0 THEN 'NO' WHEN f.facturacion_emitido=1 THEN 'SI' ELSE '' END emitido,f.facturacion_emitido,g.usuario, DATE_FORMAT(f.fec_registro, '%d/%m/%Y %H:%i') fec_registro,t.tipo_vehiculo
        ...
        WHERE date_format(f.fecha, '%Y-%m-%d') >='$fecha' AND date_format(f.fecha, '%Y-%m-%d')<='$fechafin' AND facturacion_estado=0 ") or die(mysqli_error());
    $i = 1;
    while ($row = mysqli_fetch_array($query)) {
        ...
        ?>
            ...
            <td><?php echo $row['fecha'] . ' ' . $row['horacomp']; ?></td>
...
<?php }?>
            ...
            <a class="btn btn-dark btn-print fa fa-print" href="<?php echo "ver_comprobante.php?facturacion_id=$facturacion_id"; ?>"  role="button" data-toggle="tooltip" data-placement="top" title="Imprimir Boleta"></a>

                <?php }?>
                <?php if ($row['facturacion_emitido'] == '1' && $row['fact_estado'] != 'Aceptada') {
            ?>
                <button type="button" class="btn btn-info btn-print fa fa-refresh" onClick="ReenviarBoleta('<?php echo $facturacion_id; ?>')"  role="button" data-toggle="tooltip" data-placement="top" title="Reenviar Boleta"></i></button>
            <?php }?>

<?php if ($tipo == "administrador") {?>
            <button type="button" class="btn btn-danger btn-print fa fa-trash" onClick="AnularBoleta('<?php echo $facturacion_id; ?>')"  role="button" data-toggle="tooltip" data-placement="top" title="Anular Boleta"></i></button>
<?php }?>
            </td>
            </tr>
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (response[0].success==1) {
                        ...
                        location.href='listado_facturacion.php';
                    } else {
                        alert(response[0].msg);
                    }
