...
$qempresa      = mysqli_query($con, "SELECT tolerancia FROM empresa WHERE id_empresa=1 ") or die(mysqli_error($con));
$rowempresa        = mysqli_fetch_array($qempresa);
$tolerancia = $rowempresa['tolerancia'];
...
$lugar           = $row32['lugar'];
$fec_hora_actual = date("Y-m-d H:i:s");
...
$horas_dividir=12;
//$horas_agregar=intdiv($horas_transcurridos, $horas_dividir);
$time_salida = date("H");
if ($minutos_transcurridos >= intval($tolerancia)) {
...
return;*/
mysqli_query($con, "update entradas set hora_salida='$hora_salida',time_salida='$time_salida',dia_noche='$dia_noche',fecha_salida='$fechaactual',fec_registro='$fec_hora_actual'  where codigo='$codigo'  ") or die(mysqli_error());
...
$diff       = $date1_ingreso->diff($date2_salida);
$diferencia = $diff->days;
//$diferencia         = $diferencia + $horas_agregar;

$dat_ing_huor=date("Y-m-d h:m:i", strtotime( $fec_ing_uni));
$dat_sal_huor=date("Y-m-d h:m:i", strtotime( $fec_sal_uni)) ;

$horas_pasadas = (strtotime($dat_ing_huor)-strtotime($dat_sal_huor))/60/60;
$horas_pasadas = abs($horas_pasadas); $horas_pasadas = floor($horas_pasadas);
$horas_agregar=strval(intdiv($horas_pasadas,$horas_dividir));
//$diferencia=$horas_agregar;
...
//echo "<script>document.location='estadia.php?msg=1'</script>";
echo "<script>document.location='generar_ticket.php?codigo=$codigo'</script>";
